...
class PEANUT_Agent(habitat.Agent):
    def __init__(self, args, task_config: habitat.config, model=None, sde=None, S_sde=None):
        self.model = model
        self.sde = sde
        self.S_sde = S_sde
        self._POSSIBLE_ACTIONS = task_config.TASK.POSSIBLE_ACTIONS
        self.agent_states = Agent_State(args)
        self.agent_helper = Agent_Helper(args, self.agent_states, model=self.model, sde=self.sde, S_sde=self.S_sde)
    ...
    def act(self, observations):
        ...
        if self.args.use_gt_seg:
            ...
            info['goalseg'] = observations['semantic']
        ...
        planner_inputs, full_planner_inputs = self.agent_states.update_state(obs, info)
        action = self.agent_helper.plan_act(planner_inputs, full_planner_inputs)
